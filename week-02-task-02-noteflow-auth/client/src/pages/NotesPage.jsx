...
export default function NotesPage() {
  ...
  return (
    <div className="min-h-screen">
      <header className="max-w-4xl mx-auto px-6 pt-12 pb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-ink/60">Logged in as {user?.email}</p>
        </div>
        <button
          onClick={logout}
          className="border border-ink/15 font-semibold px-4 py-2 rounded-lg hover:border-ink/40 transition-colors whitespace-nowrap"
        >
          Log out
        </button>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 pb-20 space-y-6">
        ...
        {listStatus === 'error' && (
          <div className="text-center py-16">
            ...
            <button
              onClick={loadNotes}
              className="mt-4 bg-ink text-white font-semibold px-5 py-2.5 rounded-lg"
            >
              Try again
            </button>
          </div>
