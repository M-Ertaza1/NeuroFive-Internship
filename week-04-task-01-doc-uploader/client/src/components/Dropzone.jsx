      ...
      {!file && (
        ...
        >
          ...
          <p className="mt-1 text-sm text-ink/50">
            Images, PDF, or Word — up to {MAX_SIZE_MB}MB
          </p>
          <input
            ref={inputRef}
            type="file"
      ...
      {file && (
        <div className="bg-white rounded-xl border border-ink/10 p-5 fade-in">
          <div className="flex items-center gap-4">
            {previewUrl ? (
              <img src={previewUrl} alt="Preview" className="w-16 h-16 object-cover rounded-lg border border-ink/10" />
            ) : (
              <div className="w-16 h-16 flex items-center justify-center rounded-lg bg-canvas text-2xl">
                📄
              </div>
            )}
            ...
            {status !== 'uploading' && (
              <button onClick={resetSelection} className="text-ink/40 hover:text-coral text-xl leading-none" aria-label="Remove file">
                ×
              </button>
            )}
          </div>
          ...
          {status === 'error' && <p className="mt-3 text-sm text-coral">{uploadError}</p>}
